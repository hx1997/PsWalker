...
Function DumpProcessMemoryToFile(ByVal dwPID As Long, ByVal szOutputFile As String) As Long
    On Error GoTo ReturnFunc
    Dim hProcess As Long, hFile As Long, lpAddress As Long, buf(PAGE_SIZE) As Byte, fStatus As Long
    ...
    hFile = CreateFile(szOutputFile, GENERIC_READ Or GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_NEW, 0, 0)
    
    If (hFile = INVALID_HANDLE_VALUE) Then
        GoTo ErrHandler
    End If
    
    For lpAddress = &H10000 To &H7FFFFFFF Step PAGE_SIZE
        If ReadProcessMemory(hProcess, ByVal lpAddress, buf(0), PAGE_SIZE, 0) Then
            If WriteFile(hFile, buf(0), PAGE_SIZE, 0, 0) = 0 Then
                GoTo ErrHandler
            End If
        End If
...
ReturnFunc:
    ...
    DumpProcessMemoryToFile = fStatus
End Function

Function ExecProgram(ByVal szPath As String, ByVal szCmd As String) As Boolean
    Dim sa As SECURITY_ATTRIBUTES, si As STARTUPINFO, pi As PROCESS_INFORMATION
    
    sa.nLength = Len(sa)
    si.cb = Len(si)
    
    If CreateProcess(szPath, szCmd, sa, sa, 0&, 0&, 0&, vbNullString, si, pi) Then
...
Function RestartProcess(ByVal dwPID As Long) As Boolean
    Dim szPath As String, szCmd As String
    
    szPath = StandardizeProcessPath(GetProcessImagePath(dwPID))
    szCmd = GetProcessCommandLine(dwPID)
    
    If (szPath <> "") Or (szCmd <> "") Then
        If EndProcess(dwPID) Then
            If ExecProgram(szPath, szCmd) Then
                RestartProcess = True
            End If
        End If
    End If
